...
function initAnimations() {
    ...
    const isAboutPage = window.location.pathname === '/about' || window.location.pathname.includes('/about');
    ...
    function shouldTriggerImmediately(element) {
        ...
        const firstSection = document.querySelector('.main-body > .section:first-child');
        return section === firstSection;
    }
    
    // Animation for elements with anim-1_1-items class
    const anim1Items = document.querySelectorAll('.anim-1_1-item, .anim-1_1-items > *');
    anim1Items.forEach((item, index) => {
        ...
        if (isAboutPage && item.closest('.section')?.classList.contains('brand-in-numbers')) {
            ...
            const isFirstSmallItem = binList && 
                                     item.classList.contains('bin-item') && 
                                     !item.classList.contains('bin-item-large') &&
                                     Array.from(binList.querySelectorAll('.bin-item')).indexOf(item) === 0;
    ...
    const anim2Items = document.querySelectorAll('.anim-1_2-item, .anim-1_2-items > *');
    ...
    const anim3Items = document.querySelectorAll('.anim-1_3-item, .anim-1_3-items > *');
    ...
    const anim4Items = document.querySelectorAll('.anim-1_4-item, .anim-1_4-items > *');
    anim4Items.forEach((item, index) => {
        ...
        if (isAboutPage && item.closest('.section')?.classList.contains('team')) {
            gsap.from(item, animationConfig);
        } else {
            animationConfig.scrollTrigger = {
                trigger: item,
    ...
    if (typeof SplitText !== 'undefined') {
        const headings = document.querySelectorAll('.section-headline h1, .section-headline h2, .intro-heading h2');
        headings.forEach(heading => {
            ...
            if (isAboutPage && heading.closest('.section')?.classList.contains('brand-in-numbers')) {
        ...
        if (pelElements.length > 0) {
            pelElements.forEach((pel, index) => {
                ...
                if (!isNaN(number) && number > 0) {
                    ...
                    if (isAboutPage && container.closest('.section')?.classList.contains('brand-in-numbers')) {
                        gsap.to(pel, {
                            textContent: number,
                            duration: 2,
                            ease: 'power2.out',
                            ...
                            delay: index * 0.1,
                            onUpdate: function() {
                                pel.textContent = Math.ceil(this.targets()[0].textContent);
                            }
                        });
                    } else {
                        ScrollTrigger.create({
                            trigger: container.closest('.bin-item-line, .bin-item-bordered') || container,
                            start: 'top 80%',
                            onEnter: () => {
                                gsap.to(pel, {
                                    textContent: number,
    ...
    dataNumbContainers.forEach(container => {
        ...
        if (!isNaN(number) && number > 0 && span) {
            ...
            if (isAboutPage && container.closest('.section')?.classList.contains('brand-in-numbers')) {
                ...
                const isLargeItem = parentItem?.classList.contains('bin-item-large');
                const isFirstSmallItem = parentItem?.classList.contains('bin-item') && 
                                         !parentItem.classList.contains('bin-item-large') &&
                                         parentItem.closest('.bin-list')?.querySelectorAll('.bin-item').length > 0 &&
                                         Array.from(parentItem.closest('.bin-list')?.querySelectorAll('.bin-item') || []).indexOf(parentItem) === 0;
                    ...
                    ScrollTrigger.create({
                        ...
                        onEnter: () => {
                            ...
                            gsap.to(span, {
                                ...
                                snap: { textContent: 1 },
                                onUpdate: function() {
                                    span.textContent = Math.ceil(this.targets()[0].textContent);
                                }
                            });
                        }
                    });
    ...
    // Button animations
    const buttons = document.querySelectorAll('.btn-anim, .btn-scroll-to-next-section');
    buttons.forEach(btn => {
...
if (window.location.pathname === '/about' || window.location.pathname.includes('/about')) {
    function setFirstItemsVisible() {
        ...
        if (brandSection) {
            ...
            if (largeItem) {
                gsap.set(largeItem, { opacity: 1, y: 0, immediateRender: true });
                const largeNumb = largeItem.querySelector('[data-numb] span');
                if (largeNumb) {
                    largeNumb.textContent = largeItem.querySelector('[data-numb]')?.getAttribute('data-numb') || '10';
                }
            }
            
            if (firstSmallItem) {
                gsap.set(firstSmallItem, { opacity: 1, y: 0, immediateRender: true });
                const firstNumb = firstSmallItem.querySelector('[data-numb] span');
                if (firstNumb) {
                    firstNumb.textContent = firstSmallItem.querySelector('[data-numb]')?.getAttribute('data-numb') || '28';
                }
            }
        }
    }
